...
CREATE POLICY "Users manage own chat_messages" ON public.chat_messages
    FOR ALL USING (session_id IN (SELECT id FROM public.chat_sessions WHERE user_id = auth.uid()::text));
...
CREATE POLICY "Users manage own quiz_questions" ON public.quiz_questions
    FOR ALL USING (attempt_id IN (SELECT id FROM public.exam_attempts WHERE user_id = auth.uid()::text));
