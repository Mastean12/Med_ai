...
def enforce_ai_usage(user_id: str):
    """
    Enforce freemium AI usage limits. Gracefully allows access
    if the usage tracking table is unavailable or Supabase is down.
    """
    try:
    ...
    try:
        ...
        logger.warning("Usage tracking unavailable, allowing access: %s", str(e)[:200])
