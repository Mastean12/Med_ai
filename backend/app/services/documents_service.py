...
def clean_extracted_text(text: str) -> str:
    """Clean extracted text before chunking and embedding."""
    if not text:
        return ""
...
def _content_type(ext: str) -> str:
    mapping = {".pdf": "application/pdf", ".docx": "application/vnd.openxmlformats-officedocument.wordprocessingml.document", ".txt": "text/plain"}
    return mapping.get(ext, "application/octet-stream")


async def upload_document_to_supabase(user: dict, file: UploadFile):
    sb = supabase_admin()
    user_id = user.get("id")
    if not user_id:
    ...
    # 2) Read content + upload to Storage
    try:
    ...
    # 3) Extract text + clean + chunk + insert into doc_chunks
    try:
        ...
        sb.table("documents").update({"status": "chunked"}).eq("id", document_id).execute()
        return {"document_id": document_id, "status": "chunked", "storage_path": storage_path, "chunks_created": len(chunks) if chunks else 0}
